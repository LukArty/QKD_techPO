...
int ceSerial::GetComPortNumber()
{
    ...
    // Проверяем "COM" без учёта регистра
    if (std::toupper(port_[0]) != 'C' ||
        std::toupper(port_[1]) != 'O' ||
        std::toupper(port_[2]) != 'M')
        return -1;

    // Парсим номер после COM
    int number = 0;
    for (size_t i = 3; i < port_.size(); ++i)
    {
        if (!std::isdigit(port_[i]))
            return -1;
        number = number * 10 + (port_[i] - '0');
    }
